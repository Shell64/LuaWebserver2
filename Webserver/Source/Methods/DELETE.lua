...
local function DELETE(ClientConnection, HeaderInformation, HeaderContent)
	...
	if not Found or HeaderInformation.MethodData:Find("..", nil, true) then
		...
		Log(String.Format(Language[Webserver.Language][3], ClientConnection:GetID(), ToString(IP), ToString(Port), "DELETE", ToString(Found or HeaderInformation.MethodData)))
	...
	--else, if File was found: send the data for it, if it's a compilable page (.lua) it will compile and run the code and send the returned data.
		...
		local IP, Port = ClientConnection.ClientTCP:getpeername()
		Log(String.Format(Language[Webserver.Language][4], ClientConnection:GetID(), ToString(IP), ToString(Port), "DELETE", ToString(Found)))
