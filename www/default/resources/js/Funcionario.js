...
function Cadastrar(btn_submit) {
    ...
    objeto.rg = document.getElementById("rg").value;
    objeto.cpf = document.getElementById("cpf").value;
    objeto.cargo = document.getElementById("cargo").value;
    objeto.email = document.getElementById("email").value;
    objeto.telefone = document.getElementById("telefone").value;
    objeto.usuario = document.getElementById("usuario").value;
    objeto.senha = document.getElementById("senha").value;

    $.ajax({
        type: 'POST',
        url: 'funcionario.lua?Cadastrar',
        ...
        success: function (retorno) {
            ...
            if (retorno.d) {
                ...
                document.location.href = "funcionario.html";
            }
            else {
...
function PreencherTabela() {
    $.ajax({
        type: 'POST',
        url: 'funcionario.lua?ConsultarTodos',
        data: "{}",
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        success: function (retorno) {
...
function _PreencherTabela(dados) {
    ...
    $.each(dados, function (i, item) {
        ...
        html += '<tr>' +
                    '<td class="' + classe + '">' +
                        '<a href="funcionario_alterar.html?id=' + item.id + '" target="conteudo" tabindex="-1">' +
                    ...
                    '<td class="' + classe + '">' + String(item.cargo).toUpperCase() + '</td>' +
                '</tr>';
    });

    $('#tb_funcionarios tbody').empty().append(html);
}
